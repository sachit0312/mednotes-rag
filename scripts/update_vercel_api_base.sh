...
# Sync Vercel env VITE_API_BASE_URL to current ngrok https URL and redeploy.
...
echo "Updating Vercel env VITE_API_BASE_URL for ${ENVIRONMENT}…"
vercel env rm VITE_API_BASE_URL "${ENVIRONMENT}" -y >/dev/null 2>&1 || true
printf "%s" "${NGROK_URL}" | vercel env add VITE_API_BASE_URL "${ENVIRONMENT}" -y
